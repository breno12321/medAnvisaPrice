import MongoRepository from "../providers/MongoProvider/db.repository";
...
describe('PROVIDER: Mongo provider', () => {
  it('Should return data matching the imputs when provided with real data', async (done) => {
    const res = await findFilteredMongo("REGISTRO", "1126001990026");
    expect(res).toMatchObject(response)
    done();
  })
  it('Should return no data matching the imputs when provided with wrong value', async (done) => {
    const res = await findFilteredMongo("REGISTRO", "1126001996");
    expect(res).toStrictEqual([])
    done();
  })
  it('Should return no data matching the inputs when provided with wrong filter', async (done) => {
    const res = await findFilteredMongo("REGITRO", "1126001990026");
    expect(res).toStrictEqual([])
    done();
  })
  it('Should return no data matching the inputs when provided with wrong filter', async (done) => {
    // jest.doMock("../providers/MongoProvider/db.repository")
    // const res = await findFilteredMongo("REGITRO", "1126001990026");
    // expect(res).toStrictEqual([])
    // console.log(typeof MongoRepository)
    // const mockGetResult = jest.fn(findFilteredMongo).mockRejectedValue('Error: ENOENT: no such file or directory')
    expect(findFilteredMongo("REGITRO", "1126001990026")).rejects.toEqual('Error: ENOENT: no such file or directory')
    done();
  })
})
